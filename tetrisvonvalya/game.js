...
function countPlus (lines0) { //Подсчёт очков
    ...
    document.getElementById('tetriscount').innerHTML =
        "LINES: "+lines+"<br><br>COUNT: "+count+"<br><br>RECORD: "+maxCount;
    if (this.count>500) {
        interval = setInterval (playGame,470);
    }else if (this.count>1000) {
        interval = setInterval (playGame,450);
    }
...
function keyPress( key ) { //Обработчик нажатий клавиш
    switch ( key ) {
        case 'escape':
            window.alert ('Standby mode!'); //В JS уже есть модальное окно :)
            break;
        case 'left':
            if (valid(-1)) --currentX;
            break;
...
function playGame() { //Контроль падения фигурки, создание новой и очистка линии
    if (valid(0,1)) currentY++;
        ...
        if (lose) {
            window.alert ('Your Record: '+maxCount);
            newGame();
            return false;
        }
        newShape();
...
function newGame() { //Новая игра
    ...
    interval = setInterval (playGame,500); //скорость игры, мс
}
